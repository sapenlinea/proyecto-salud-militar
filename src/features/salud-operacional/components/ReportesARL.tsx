...
import type { ReporteARL } from '../types';

interface ReportesARLProps {
  reportes: ReporteARL[];
}
...
export default function ReportesARL({ reportes }: ReportesARLProps) {
  ...
  return (
    <Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Reportes a la ARL. Accidentes, incapacidades, evaluaciones y periódicos.
      </Typography>
      ...
      <Table size="small">
        <TableHead>
          <TableRow>
            ...
            <TableCell align="right">Accidentes</TableCell>
        ...
        <TableBody>
          {filtered.map((r) => (
            <TableRow key={r.id}>
              ...
              <TableCell align="right">{r.accidentCount ?? '—'}</TableCell>
              <TableCell align="right">{r.evaluationCount ?? '—'}</TableCell>
              <TableCell>
                <Chip

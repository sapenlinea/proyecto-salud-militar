...
import type { ReportePAI } from '../types';

interface ReportesPAIProps {
  reportes: ReportePAI[];
}
...
export default function ReportesPAI({ reportes }: ReportesPAIProps) {
  ...
  return (
    <Box>
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Reportes PAI (Programa Ampliado de Inmunización). Mensuales, trimestrales y anuales.
      </Typography>
      ...
      <Table size="small">
        <TableHead>
          <TableRow>
            ...
            <TableCell>Vacunas incluidas</TableCell>
            <TableCell align="right">Dosis aplicadas</TableCell>
        ...
        <TableBody>
          {filtered.map((r) => (
            <TableRow key={r.id}>
              ...
              <TableCell sx={{ maxWidth: 240 }}>{r.vaccinesIncluded.join(', ')}</TableCell>
              <TableCell align="right">{r.totalDosesApplied}</TableCell>
              <TableCell>
                <Chip
